...
| 브라우저를 옮겨 온 방문(추천 아님) | 카카오톡 같은 앱 안 브라우저에서 본인이 **기본 브라우저로 옮겨 연 것**이에요. 소스 `inapp`은 ‘기본 브라우저로 열기’ 버튼(첫 화면 안내·소리 배너·로그인 도움 창), `inapp_copy`는 ‘🔗 주소 복사’로 옮긴 경우예요. 둘 다 매체가 `browser_switch`로 따로 잡히니 추천 수에서 빼고 보세요 |
| 로그인한 사용자 수 | Authentication → 사용자 |
| 한 사람의 기록 | Firestore → `users/{uid}` (앱이 쓰는 형식 그대로라 직접 고치지 마세요) |
| 삭제 요청 처리 | 사용자가 앱에서 못 지울 때: Firestore → `users/{uid}` 문서 삭제 **+** Authentication → 사용자 → 그 계정 ⋮ → **계정 삭제**(이름·이메일·프로필 사진 주소·가입/마지막 로그인 시각까지 지워짐). `uid`는 Authentication 사용자 목록에서 이메일로 찾아요. 의견 삭제 요청은 `feedback`에서 글·보낸 날(`at`)·`uid`로 찾아 문서를 지워요 |
| 의견 1년 보관 지키기 | 개인정보 안내는 ‘의견은 늦어도 1년 안에 지운다’고 약속해요. **매달 첫 주**에 Firestore → `feedback`에서 `at`이 **10개월** 넘은 문서를 지워요(`at` 기준으로 필터·정렬 → 문서 ⋮ → 문서 삭제). 분기마다(1·4·7·10월 첫 주) 한다면 기준을 **8개월**로 잡아요. 기준을 ‘1년’으로 두면 다음 정리 때까지 기다리는 동안 1년을 넘겨요(분기마다 하면 최대 약 15개월). ⚠️ Firestore TTL 정책은 칸의 시각이 **지나면** 지우는 방식이라 `at`(보낸 시각)에 걸면 곧바로 지워져요. TTL로 자동화하려면 앱이 `expireAt`(보낸 시각 + 11개월 — TTL은 시각이 지난 뒤 며칠 안에 지우므로 여유를 둠)을 함께 저장하도록 바꾸고 `firestore.rules`의 `feedback` 허용 칸에도 더한 뒤, 그 칸에 TTL을 거세요 |
...
- [ ] `CLOUD_CONFIG`에 `since: 'YYYY-MM-DD'`(켠 날짜)를 적었나요? 개인정보 안내의 적용일과 변경 이력(‘… 구글 로그인·클라우드 저장·의견 보내기·사용 통계 도입’)이 이 날짜로 나와요. 적지 않거나 달력에 없는 날짜·다른 형식(예: `2026.10.01`, `2026-02-30`)이면 무시하고 처음 만든 날(2026-09-25)로 보여 주며, 브라우저 콘솔에 경고를 남겨요.
