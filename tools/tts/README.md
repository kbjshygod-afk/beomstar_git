...
- mp3를 손으로 바꿔 넣었다면(도구 밖에서 다시 녹음 등) `index.json`의 `gen`을 1 올린 뒤 `node tools/tts/generate.mjs --lang xx --limit 0`으로 `rev`를 다시 계산해 주세요. 그러지 않으면 이미 들은 사용자에게는 옛 파일이 계속 나와요.
...
- 한글이 섞인 문자열 — 선택 문제 보기 `ㅃ`(zh), `ㅅ`(fr) 등. 앱도 `/[ㄱ-ㅎㅏ-ㅣ가-힣]/` 검사로 자모를 포함해 한글이 섞인 정답 보기는 읽지 않아요(`clips.mjs`의 `HANGUL`이 같은 조건).

## 앱에서 어떻게 쓰이나 (language-teacher/index.html)

1. 언어를 열 때 `audio/{lang}/index.json`을 받아(`loadClips`, 8초 안에 못 받으면 이번에는 기기 음성) `ids`를 `Set`으로 들고 있어요. 파일이 없으면(404) 그 언어는 지금처럼 기기 음성만 써요.
2. `speak(text, rate, speaker)`·`speakSeq()`는 `slot = (speaker === 'B' || speaker === 'D') ? 'b' : 'a'`, `id = clipId(slot, text)`로 찾아요(`clipFor`/`clipUrl`). `b`가 없으면 `a`를 써요.
3. 있으면 `fetch`로 받아 한 개의 `<audio>`로 재생(`playClip`), 없거나 못 받거나 재생이 막히면 **같은 문장을 기기 음성(`speechSynthesis`)으로** 읽어요.
   - 응답이 멈춘 망·느린 망: 2.5초(`CLIP_WAIT`) 안에 재생이 시작되지 않으면 그 문장은 기기 음성으로 읽고, 늦게 받아진 녹음은 틀지 않아요. 받기는 15초(`CLIP_GIVEUP`)까지 뒤에서 이어 가서(다음에 누르면 바로 들림), 그래도 안 오면 버려서 그 문장이 계속 막히지 않아요.
   - 녹음을 못 틀어 기기 음성으로 바꿀 때도 클라우드 음성(크롬 'Google …' 등) 오류 → 기기에 든 음성으로 다시 읽기는 그대로 돼요.
   - '소리가 안 나는 기기' 판정(`ttsBadNow` → 듣기 문제의 🔇 글자로 보기, 먼저 듣기의 대본 바로 열기): 녹음이 잘 재생되는 동안은 기기 음성 상태와 관계없이 소리가 난다고 보고, 녹음 재생이 실패하면(다시 성공할 때까지) 기기 음성 상태로 판단해요.
4. 속도: `playbackRate = 요청 속도 ÷ 그 목소리 칸의 녹음 속도(index.json의 rates) × 사용자 속도`, 음높이 유지(`preservesPitch`).
5. 주소는 `audio/{lang}/{id}.mp3?v={index.json의 rev}` — 목소리·속도를 바꾸거나 `--redo`로 다시 녹음하면 `rev`가 바뀌어 캐시가 저절로 바뀌어요(`rev`가 없는 옛 목록은 목소리·속도로 만들어요).
6. 서비스 워커:
   - index.json은 네트워크 우선(저장본이 있으면 3초까지 기다림), 못 받으면 저장본. 허브·공유 링크(`?lang=`)로 온 첫 방문처럼 서비스 워커가 켜지기 전에 받은 목록도 페이지가 알려 줘서(`warm`) `lt-audio`에 담겨요.
   - mp3는 한 번 들은 것만 `lt-audio` 캐시에 담겨 오프라인에서도 들려요. 담을 때 캐시를 훑지 않고, index.json의 `rev`가 바뀌었을 때만 그 언어의 옛 버전 칸을 한 번에 지워요.
